...
const Timeline_Element = ({ content }) => {
  ...
  return (
    <li id={content.title + '-parent'}>
      ...
      <div id={content.title} className="timeline-start mb-10 md:text-end rounded-sm shadow-xl overflow-hidden tansition transition-all duration-2000 translate-y-50 opacity-0">
        {
        content.link !== null ?
        <Link href={content.link} className=''>
          <Image src={content.src} width={1000} height={1000} alt={content.title} className='hover:scale-125 transition transition-all duration-1500 ease-in-out' />
        </Link> 
        : <Image src={content.src} width={1000} height={1000} alt={content.title} className='hover:scale-125 transition transition-all duration-1500 ease-in-out' />
        }
      </div>
