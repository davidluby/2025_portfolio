...
import Fluid from './2D-Simulator'
import Initialize from './Initial-Conditions'
  ...
  useEffect (() => {
    ...
    if (tag === undefined) {
      ...
      hero.addEventListener('touchstart', event => {
        ...
        startDrag(event.touches[0].clientX, event.touches[0].pageY - rect.top)
      });
      ...
      hero.addEventListener('touchmove', event => {
        ...
        event.stopImmediatePropagation();
        let rect = event.target.getBoundingClientRect()
        drag(event.touches[0].clientX, event.touches[0].pageY - rect.top)
      }, { passive: false});
  }
